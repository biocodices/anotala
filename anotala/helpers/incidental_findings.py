...
def get_omim_incidental_genes_and_phenos():
    ...
    if not isfile(fn):
        ...
        print(f'ACMG file not cached for this month. Fetching from: {url}')
        ...
        try:
            df = _parse_incidental_findings(df)
        except Exception:
            unparsed_fn = fn.replace('.csv', '.unparsed.csv')
            df.to_csv(unparsed_fn)
            print(f'Failed parsing. Dumped the unparsed CSV in:\n{fn}')
            print(df.head())
            raise
...
def _parse_incidental_findings(df):
    # Seems this is not necessary anymore:
    # df = _use_first_row_as_header(df)
    df = fix_df_colspan_issues(df)
    ...
    df.loc[no_gene, 'gene'] = \
        df.loc[no_gene, 'MedGen'].str.extract(r'MedGen\s+(\w+) \(',
                                              expand=False)
...
def _use_first_row_as_header(df):
