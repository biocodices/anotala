...
class PostgresCache(Cache):
    ...
    def __init__(self, credentials_filepath=CREDS_FILE):
        ...
        logger.info('Connected to {} ({!r})'.format(self.__class__.__name__,
                                                    self.engine.url))
        self.tables = {}
    ...
    def _client_get(self, ids, namespace, load_as_json):
        ...
        # JSON field type. After this, the psql client automatically deals with
        # [de]serialization of JSON, so the setting and retrieval of data is
        # the same wether the field is JSON or not.
        table = self._get_table(namespace, as_json=load_as_json)
    ...
    def _client_set(self, info_dict, namespace, save_as_json):
        # The argument load_as_json is only relevant the first time an
        # annotator is used, in self._get_table(), to create the table with a
        # JSON field type. After this, the psql client autoatically deals with
        # [de]serialization of JSON, so the setting and retrieval of data is
        # the same wether the field is JSON or not.
        table = self._get_table(namespace, as_json=save_as_json)
    ...
    def _get_table(self, tablename, as_json=False):
        ...
        if tablename not in self.tables:
            self.tables[tablename] = self._create_table(tablename, as_json)
