...
def test_gwas_catalog_annotator_parse_annotation():
    ...
    assert GwasCatalogOldAnnotator._parse_annotation(raw) is None
    ...
    parsed_data = GwasCatalogOldAnnotator._parse_annotation(raw)
...
def test_infer_allele(rsid_allele, expected_tuple):
    assert GwasCatalogOldAnnotator._infer_allele(rsid_allele) == expected_tuple
...
def test_parse_pubmed_entries(association_entry, expected_pubmed_entries):
    result = GwasCatalogOldAnnotator._parse_pubmed_entries(association_entry)
...
def test_parse_pubmed_entries_raise():
    with pytest.raises(Exception):
        association = {'author_year_id': ['John Doe|2017|PM1',
                                          'Johnny D|2017|PM1']}
        GwasCatalogOldAnnotator._parse_pubmed_entries(association)
...
def test_parse_sample_size(given_list, expected_dict):
    result = GwasCatalogOldAnnotator._parse_sample_size(given_list)
...
def test_parse_entrez_mapped_gene_link(gene_link, expected_result):
    result = GwasCatalogOldAnnotator._parse_entrez_mapped_gene_link(gene_link)
    assert result == expected_result


def test_parse_reported_gene_link():
    gene_link = 'GENE1|1|ENSG01'
    result = GwasCatalogOldAnnotator._parse_reported_gene_link(gene_link)
...
def test_parse_ancestry_link(ancestry_link, expected_info):
    parsed_info = GwasCatalogOldAnnotator._parse_ancestry_link(ancestry_link)
...
def test_group_sample_info():
    ...
    grouped = GwasCatalogOldAnnotator._group_sample_info(sample_info)
...
def test_parse_colon_separated_values(values, expected_result):
    result = GwasCatalogOldAnnotator._parse_colon_separated_values(values)
    assert result == expected_result


def test_parse_ci_range():
    f = GwasCatalogOldAnnotator._parse_ci_range

    assert f('[NR]') is None
    assert f('[-0.1-1.5]') == {'lower_limit': -0.1, 'upper_limit': 1.5}
    assert f('[1.72-4]') == {'lower_limit': 1.72, 'upper_limit': 4}
