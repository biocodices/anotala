...
def test_snpeff_annotator_parse_hit():
    ...
    assert annotations[0]['allele'] == 'T'

    hit_single['snpeff']['ann']['hgvs_c'] = 'del123CCC'
    annotations = SnpeffAnnotator._parse_hit(hit_single)
    assert annotations[0]['allele'] == 'del123CCC'

    hit_multiple = {'snpeff': {'ann': [{'effect': 'eff-3'},
                                       {'effect': 'eff-3'},
                                       {'effect': 'eff-4'}]}}
